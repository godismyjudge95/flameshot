...
void CaptureWidget::initHelpMessage()
{
    ...
    keyMap << QPair<QString, QString>(tr("Mouse"),
                                      tr("Select screenshot area"));
    ...
    keyMap << QPair<QString, QString>(tr("Mouse Wheel"),
                                      tr("Change tool size"));
    keyMap << QPair<QString, QString>(tr("Right Click"),
                                      tr("Show color picker"));
    keyMap << QPair<QString, QString>(
      ConfigHandler().shortcut("TYPE_TOGGLE_PANEL"), tr("Open side panel"));
...
void CaptureWidget::mousePressEvent(QMouseEvent* e)
{
    ...
    if (m_selection->getMouseSide(e->position().toPoint()) !=
    ...
    // of it
    if (m_toolWidget &&
        !m_toolWidget->geometry().contains(e->position().toPoint())) {
        commitCurrentTool();
        m_panel->setToolWidget(nullptr);
        drawToolsData();
        updateLayersPanel();
...
void CaptureWidget::mouseMoveEvent(QMouseEvent* e)
{
    ...
    if (!m_activeButton && m_panel->activeLayerIndex() >= 0) {
        ...
        if (m_startMove) {
            ...
            if (m_activeToolOffsetToMouseOnStart.isNull()) {
                ...
                m_activeToolOffsetToMouseOnStart =
                  e->position().toPoint() - *activeTool->pos();
            }
            ...
            activeTool->move(e->position().toPoint() -
                             m_activeToolOffsetToMouseOnStart);
            drawToolsData();
        }
        ...
        if (m_adjustmentButtonPressed) {
            ...
            m_activeTool->drawMove(m_displayGrid
                                     ? snapToGrid(e->position().toPoint())
                                     : e->position().toPoint());
        }
